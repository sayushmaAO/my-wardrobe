(() => {
  ...
  function renderPersonal(filter = 'all') { activeFilter = filter; visibleLooks = allLooks().filter(x => filter === 'all' || x.tag === filter || filter === 'favorite' && x.favorite); document.querySelector('#lookGrid').innerHTML = visibleLooks.map(imageCard).join(''); }
  ...
  function addPhotoButtons() {
    ...
    document.querySelectorAll('#closetGrid .item-card:not(.user-item)').forEach((card, index) => {
      ...
      if (replacements[index]) media.innerHTML = `<img src="${replacements[index]}" alt="${name}"><button class="change-item change-static" data-change-static="${index}" aria-label="Изменить фото">↻</button>`;
  ...
  function renderItems() {
    ...
    if (added.length) grid.insertAdjacentHTML('afterbegin', added.map((item, i) => `<article class="item-card user-item" data-kit="${i}"><div class="item-image photo-item"><img src="${item.src}" alt="${item.name}"><button class="change-item" data-change-item="${i}" aria-label="Изменить фото">↻</button></div><h3>${item.name}</h3><p>моя вещь · нажми для комплекта</p></article>`).join(''));
    addPhotoButtons();
  }
  function renderInspiration() { const data = get('wardrobe:inspiration'); document.querySelector('#inspirationGrid').innerHTML = data.map((x, i) => `<article class="inspo-card"><img src="${x.src}" alt="Вдохновение"><button data-remove-inspo="${i}" aria-label="Удалить">×</button><p>${x.name}</p></article>`).join('') || '<p class="empty-state">Здесь будут твои референсы: фото образов, деталей и сочетаний.</p>'; }
  ...
  async function addFiles(files, target, kind) {
    ...
    for (const file of selected) {
      try {
        ...
        const entry = kind === 'item' ? { id: `user:${Date.now()}-${Math.random().toString(16).slice(2)}`, name, src, note: 'Новая вещь', tag: 'work', custom: true } : kind === 'inspiration' ? { name, src } : { id: `user:${Date.now()}-${Math.random().toString(16).slice(2)}`, name, src, note: 'Добавь повод и детали', tag: 'work', custom: true };
        list.unshift(entry);
