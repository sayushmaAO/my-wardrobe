(() => {
  ...
  function renderItems() {
    ...
    if (filtered.length) grid.insertAdjacentHTML('afterbegin', filtered.map(({ item, i }) => `<article class="item-card user-item" data-kit="${i}"><div class="item-image photo-item"><img src="${item.src}" alt="${item.name}"><button class="change-item" data-change-item="${i}" aria-label="Изменить фото">↻</button></div><h3>${item.name}</h3><p><button class="item-tag" data-edit-category="${i}">${item.categoryLabel || 'Другое'}</button> · моя вещь</p></article>`).join(''));
    addPhotoButtons();
  }
